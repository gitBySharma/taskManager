...
exports.postAddTask = async (req, res, next) => {
    ...
    try {
        const { title, description, status, dueDate } = req.body;  //destructure request body

        //validate task status
        if (status !== "pending" && status !== "inProgress" && status !== "completed") {
            t.rollback();  //rollback transaction if status invalid

            return res.status(400).json({ message: "Invalid status, Status must be 'pending', 'inProgress' or 'completed'", success: false });
...
exports.getAllTasks = async (req, res, next) => {
    ...
    try {
        ...
        if (title) {
            taskFilterCriteria = { [Sequelize.Op.like]: `%${title}%` };   //checks for like strings as the title
        }
        if (status) {
            taskFilterCriteria.status = status;
        }
        if (dueDate) {
            taskFilterCriteria.dueDate = { [Sequelize.Op.eq]: dueDate };
        }
...
//controller to fetch a particular task
exports.getSpecificTask = async (req, res, next) => {
    try {
        const id = req.params.id;
        const task = await Tasks.findOne({ where: { id: id, userId: req.user.id } });
        if (task) {
            res.status(200).json({ task: task, message: "Task fetched successfully", success: true });
        ...
        console.log("Specific task fetching error - ", error);
        res.status(500).json({ Error: "Something went wrong", success: false });
    }
};
...
exports.updateTask = async (req, res, next) => {
    ...
    try {
        ...
        if (task) {
            ...
            // await task.save();
            const updatedTask = await task.update({ title, description, status, dueDate }, { transaction: t });  //update the task with new values
